...
export default function DigitalCardPage() {
  ...
  return (
    <div className="flex-grow flex flex-col bg-surface md:bg-[#F8FAFF] text-on-surface font-body-md relative pb-24 md:pb-8 min-h-screen">
      ...
      {/* Main Card View */}
      <main className="flex-grow overflow-y-auto w-full max-w-6xl mx-auto p-4 md:p-8 animate-fade-in">
        {/* Desktop title action bar */}
        <div className="hidden md:flex items-center justify-between border-b border-outline-variant/30 pb-5 mb-6">
          <div>
            <h2 className="text-2xl lg:text-3xl font-black text-on-surface tracking-tight">Digital Health Card</h2>
            <p className="text-sm text-on-surface-variant mt-1">Show this card or QR code at reception to claim benefits.</p>
          </div>
          <button 
            onClick={() => navigate(-1)} 
            className="flex items-center gap-1.5 px-4 py-2 border border-outline-variant rounded-xl hover:bg-surface-variant transition-all cursor-pointer text-xs font-bold text-on-surface"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 md:gap-8 items-start max-w-md mx-auto md:max-w-none">
          {/* Left Column: Card Display & Actions */}
          <div className="space-y-6 md:col-span-6 lg:col-span-5">
            {/* Card Canvas */}
            <div 
              id="medcred-health-card-canvas"
          ...
          {/* Right Column: QR Scanner & Info details */}
          <div className="space-y-6 mt-6 md:mt-0 md:col-span-6 lg:col-span-7">
            {/* QR Focus Card */}
            <div className="bg-white border border-outline-variant/40 rounded-2xl p-5 flex flex-col items-center justify-center space-y-3 shadow-sm md:rounded-3xl md:shadow-[0_8px_30px_rgb(0,0,0,0.02)]">
            ...
            <div className="grid grid-cols-2 gap-3">
              {/* Plan Card */}
              <div className="bg-white border border-outline-variant/40 rounded-xl p-3.5 space-y-1 md:rounded-2xl md:shadow-[0_8px_30px_rgb(0,0,0,0.02)]">
